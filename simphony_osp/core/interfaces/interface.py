...
from typing import Iterator, Dict, Optional, Set, TYPE_CHECKING, \
    Tuple

from rdflib import Graph
from rdflib.graph import ReadOnlyGraphAggregate, ModificationException
...
from simphony_osp.core.utils.datatypes import Triple, Pattern

if TYPE_CHECKING:
    from simphony_osp.core.ontology.entity import OntologyEntity

__all__ = ["BufferType", "BufferedSimpleMemoryStore", "Interface",
           "InterfaceDriver"]
...
class BufferedSimpleMemoryStore(SimpleMemory):
    ...
    def remove(self,
        ...
        existing_triples = ((s, p, o)
                            for (s, p, o), ctx
                            in self.triples(triple_pattern))
    ...
    def triples(self,
                triple_pattern: Pattern,
                context=None,
                ignore_buffers=False) -> Iterator[Tuple[Triple,
                                                        Iterator[Graph]]]:
        ...
        triple_pool = filter(
            lambda x: x not in chain(
                self._buffers[BufferType.ADDED].triples(
                    triple_pattern),
                self._buffers[BufferType.ADDED].triples(
                    triple_pattern)
            ),
            ((s, p, o)
             for (s, p, o), ctx
             in super().triples(triple_pattern, None)),
        )
...
class InterfaceDriver(Store):
    ...
    def __init__(self,
        ...
        self._buffer_uncaught = {buffer_type: Graph(SimpleMemory())
                                 for buffer_type in BufferType}
        self._buffer_caught = {buffer_type: Graph(SimpleMemory())
                               for buffer_type in BufferType}
    ...
    def open(self, configuration: str, create: bool = False) -> None:
        ...
        self.interface.session_base = Session(base=self.interface.base,
        ...
        try:
            session.lock()
            # Session must not close when exiting its context manager.
            with session:
    ...
    def remove(self,
        ...
        for buffer in (self._buffer_uncaught, self._buffer_caught):
            ...
            existing_triples = self.interface.base.triples(triple_pattern) \
                if buffer is self._buffer_uncaught else \
    ...
    def triples(self,
                triple_pattern: Pattern,
                context=None,
                ignore_buffers=False) -> Iterator[Tuple[Triple, Graph]]:
        ...
        if hasattr(self.interface, 'triples'):
            ...
            def query_method(pattern):
                return (
                    x.triples
                    for x in self.interface.load(pattern[0])
                )
        else:
    ...
    def __len__(self, context: Graph = None,
                ignore_buffers=False) -> int:
    ...
    def query(self,
        ...
        if sum(
                len(x) for x in (
                    buffer[y]
                    for buffer in (self._buffer_uncaught,
                                   self._buffer_caught)
    ...
    def commit(self) -> None:
        ...
        self.interface.session_base = Session(base=self.interface.base,
                                              ontology=self._ontology)
        self.interface.session_old = Session(base=self.interface.old_graph,
                                             ontology=self._ontology)
        self.interface.session_new = Session(base=self.interface.new_graph,
                                             ontology=self._ontology)

        # Computes updated, added and deleted entities if enabled.
        if not self.interface.disable_entity_tracking:
            added_entities, updated_entities, deleted_entities = \
    ...
    def _compute_entity_modifications(self) -> Tuple[Set['OntologyEntity'],
                                                     Set['OntologyEntity'],
                                                     Set['OntologyEntity']]:
        ...
        class Tracker:
            ...
            def __call__(self, subject: Node):
                if subject not in self.visited_subjects:
                    if next(self._interface.old_graph.triples(
        ...
        # Get added subjects.
        for s in chain(self._buffer_uncaught[BufferType.ADDED].subjects(),
        ...
        for s in chain(self._buffer_uncaught[BufferType.DELETED].subjects(),
        ...
        deleted_subjects = {
            s: True
            if count >= sum(1 for _ in self.interface.old_graph.triples(
        ...
        updated_subjects = tracker.existing_subjects.difference(
            deleted_subjects)
        added_entities = set(self.interface.session_new.from_identifier(s)
                             for s in
                             added_subjects)
        updated_entities = set(self.interface.session_new.from_identifier(s)
                               for s in updated_subjects)
        deleted_entities = set(self.interface.session_old.from_identifier(s)
    ...
    class UnbufferedInterfaceDriver(Store):
        ...
        def triples(self,
            ...
            yield from self._driver.triples(triple_pattern, context,
                                            ignore_buffers=True)
