...
def _serialize_session_triples(
    ...
    custom_datatype_triples = (
        get_custom_datatype_triples(session)
...
def _serialize_individual_json(
    individual,
    rel=cuba_namespace.activeRelationship,
    max_depth=float("inf"),
    json_dumps=True,
):
...
def _serialize_individual_triples(
    ...
    graph = Graph()
    graph.add(
        (cuba_namespace._serialization, RDF.first, individual.identifier)
    )
...
def delete_cuds_object_recursively(
    cuds_object, rel=cuba_namespace.activeRelationship, max_depth=float("inf")
):
...
def export_cuds(
    ...
    rel: "OntologyRelationship" = cuba_namespace.activeRelationship,
...
def post(
    ...
    Returns:
        Server response
    """
    serialized = _serialize_individual_json(
