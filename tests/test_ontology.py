...
class TestOntology(unittest.TestCase):
    ...
    def test_overlay_cuba_triples(self):
        ...
        self.graph.add((URIRef("has_part"), RDF.type, OWL.ObjectProperty))
        self.graph.add((URIRef("encloses"), RDF.type, OWL.ObjectProperty))
        self.ontology.active_relationships = (
            URIRef("has_part"),
            URIRef("encloses"),
        )
        self.ontology._update_overlay()
        self.assertEqual(
            set(self.ontology.graph) - set(self.graph),
            {
                (
                    URIRef("encloses"),
                    RDFS.subPropertyOf,
    ...
    def test_overlay_default_rel_triples(self):
        ...
        self.ontology._update_overlay()
        self.assertIn(
            (URIRef("ns1"), rdflib_cuba._default_rel, URIRef("has_part")),
            self.ontology.graph,
        )
