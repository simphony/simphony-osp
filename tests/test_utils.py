...
from osp.core.utils.schema_validation import (
    CardinalityError,
    ConsistencyError,
    validate_tree_against_schema,
)
from osp.core.utils.simple_search import (
    find_cuds_object,
...
class TestUtils(unittest.TestCase):
    ...
    def test_get_custom_datatypes(self):
        ...
        self.assertIn(
            rdflib_cuba["_datatypes/VECTOR-INT-2"], get_custom_datatypes()
        )
    ...
    def test_validate_tree_against_schema(self):
        ...
        schema_file = os.path.join(
            os.path.dirname(__file__), "test_validation_schema_city.yml"
        )
        ...
        self.assertRaises(
            CardinalityError, validate_tree_against_schema, c, schema_file
        )
    ...
    def test_branch(self):
        ...
        self.assertEqual(
            {"Herdern", "Vauban"},
            set(map(lambda x: x.name, x.get(oclass=city.Neighborhood))),
        )
        self.assertEqual(
            {"Peter", "Maria"},
            set(map(lambda x: x.name, x.get(rel=city.hasInhabitant))),
        )
    ...
    def test_post(self):
        ...
        serialized = serializable(
            [c, p1, p2, p3, n1, n2, s1], partition_cuds=False, mark_first=True
        )
        assertJsonLdEqual(self, serialized, response.json())

        response = post("http://dsms.com", c, max_depth=1)
        serialized = serializable(
            [c, p1, p2, p3, n1, n2], partition_cuds=False, mark_first=True
        )
        assertJsonLdEqual(self, serialized, response.json())
    ...
    def test_create_recycle(self):
        ...
        with TestWrapperSession() as session:
            ...
            self.assertEqual(
                session._buffers,
                [[dict(), dict(), dict()], [{b.uid: b}, dict(), dict()]],
            )
            ...
            self.assertEqual(
                set(default_session._registry.keys()), {a.uid, x.uid}
            )
            self.assertIs(default_session._registry.get(a.uid), a)
            self.assertEqual(
                session._buffers,
                [[{x.uid: x}, {c.uid: c}, dict()], [dict(), dict(), dict()]],
            )
    ...
    def test_create_from_cuds_object(self):
        ...
        with TestWrapperSession() as session:
            ...
            self.assertEqual(
                c._neighbors[city.hasInhabitant], {y.uid: [city.Citizen]}
            )
            self.assertEqual(
                set(default_session._registry.keys()), {a.uid, x.uid, y.uid}
            )
            self.assertIs(default_session._registry.get(a.uid), a)
            self.assertEqual(
                session._buffers,
    ...
    def test_change_oclass(self):
        ...
        self.assertEqual(
            p1._neighbors[city.INVERSE_OF_hasInhabitant],
            {c.uid: [city.PopulatedPlace]},
        )
        self.assertEqual(
            p2._neighbors[city.INVERSE_OF_hasInhabitant],
            {c.uid: [city.PopulatedPlace]},
        )
    ...
    def test_check_arguments(self):
        ...
        self.assertRaises(TypeError, check_arguments, Cuds, city.City)
    ...
    def test_find_cuds_object(self):
        ...
        c, p1, p2, p3, n1, n2, s1 = get_test_city()
        self.assertIs(
            find_cuds_object(find_maria, c, cuba.activeRelationship, False), p3
        )
        self.assertIs(
        ...
        all_found = find_cuds_object(
            find_non_leaves, c, cuba.activeRelationship, True, max_depth=1
        )
    ...
    def test_find_cuds_object_by_uid(self):
        ...
        c, p1, p2, p3, n1, n2, s1 = get_test_city()
        self.assertIs(
            find_cuds_object_by_uid(c.uid, c, cuba.activeRelationship), c
        )
        self.assertIs(
            find_cuds_object_by_uid(p1.uid, c, cuba.activeRelationship), p1
        )
        self.assertIs(
            find_cuds_object_by_uid(p2.uid, c, cuba.activeRelationship), p2
        )
        self.assertIs(
            find_cuds_object_by_uid(p3.uid, c, cuba.activeRelationship), p3
        )
        self.assertIs(
            find_cuds_object_by_uid(n1.uid, c, cuba.activeRelationship), n1
        )
        self.assertIs(
            find_cuds_object_by_uid(n2.uid, c, cuba.activeRelationship), n2
        )
        self.assertIs(
            find_cuds_object_by_uid(s1.uid, c, cuba.activeRelationship), s1
        )
        self.assertIs(find_cuds_object_by_uid(c.uid, c, city.hasInhabitant), c)
        self.assertIs(
            find_cuds_object_by_uid(p1.uid, c, city.hasInhabitant), p1
        )
        self.assertIs(
            find_cuds_object_by_uid(p2.uid, c, city.hasInhabitant), p2
        )
        self.assertIs(
            find_cuds_object_by_uid(p3.uid, c, city.hasInhabitant), p3
        )
        self.assertIs(
            find_cuds_object_by_uid(n1.uid, c, city.hasInhabitant), None
        )
        self.assertIs(
            find_cuds_object_by_uid(n2.uid, c, city.hasInhabitant), None
        )
        self.assertIs(
            find_cuds_object_by_uid(s1.uid, c, city.hasInhabitant), None
        )
        self.assertIs(
            find_cuds_object_by_uid(c.uid, n1, cuba.activeRelationship), None
        )
        self.assertIs(
            find_cuds_object_by_uid(p1.uid, n1, cuba.activeRelationship), None
        )
        self.assertIs(
            find_cuds_object_by_uid(p2.uid, n1, cuba.activeRelationship), p2
        )
        self.assertIs(
            find_cuds_object_by_uid(p3.uid, n1, cuba.activeRelationship), p3
        )
        self.assertIs(
            find_cuds_object_by_uid(n1.uid, n1, cuba.activeRelationship), n1
        )
        self.assertIs(
            find_cuds_object_by_uid(n2.uid, n1, cuba.activeRelationship), None
        )
        self.assertIs(
            find_cuds_object_by_uid(s1.uid, n1, cuba.activeRelationship), s1
        )
    ...
    def test_find_cuds_objects_by_oclass(self):
        ...
        found = find_cuds_objects_by_oclass(
            city.Citizen, c, cuba.activeRelationship
        )
        self.assertEqual(len(found), 3)
        self.assertEqual(set(found), {p1, p2, p3})
        found = find_cuds_objects_by_oclass(
            city.Neighborhood, c, cuba.activeRelationship
        )
    ...
    def test_find_cuds_objects_by_attribute(self):
        ...
        self.assertEqual(
            find_cuds_objects_by_attribute(
                "name", "Maria", c, cuba.relationship
            ),
            [p3],
        )
        found = find_cuds_objects_by_attribute("age", 25, c, cuba.relationship)
        self.assertEqual(len(found), 3)
        self.assertEqual(set(found), {p1, p2, p3})
    ...
    def test_find_relationships(self):
        ...
        found = find_relationships(
            city.INVERSE_OF_hasInhabitant, c, cuba.relationship, False
        )
        self.assertEqual(set(found), {p1, p2, p3})
        self.assertEqual(len(found), 3)
        found = find_relationships(city.isPartOf, c, cuba.relationship, True)
        self.assertEqual(set(found), {p3, n1, n2, s1})
        self.assertEqual(len(found), 4)
        found = find_relationships(city.isPartOf, c, cuba.relationship, False)
    ...
    def test_get_relationships_between(self):
        ...
        self.assertEqual(
            get_relationships_between(p, c), {city.INVERSE_OF_hasInhabitant}
        )
