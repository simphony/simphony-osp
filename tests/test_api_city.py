...
class TestAPICity(unittest.TestCase):
    ...
    def test_recursive_add(self):
        ...
        self.assertEqual(set(c[CITY.HAS_INHABITANT].keys()),
                         set([p1.uid, p2.uid, p3.uid]))
        self.assertEqual(set([p2.uid]),
                         set(p1[CITY.HAS_CHILD].keys()))
        self.assertEqual(set([p2.uid]),
                         set(p3[CITY.HAS_CHILD].keys()))
        self.assertEqual(set([p4.uid]), set(
            p2[CITY.HAS_CHILD].keys()))

        # check passive relationships
        self.assertEqual(set([c.uid]),
                         set(p1[CITY.IS_INHABITANT_OF].keys()))
        self.assertEqual(set([p1.uid, p3.uid]),
                         set(p2[CITY.IS_CHILD_OF].keys()))
        self.assertEqual(set([c.uid]),
                         set(p2[CITY.IS_INHABITANT_OF].keys()))
        self.assertEqual(set([c.uid]),
                         set(p3[CITY.IS_INHABITANT_OF].keys()))
        self.assertEqual(set([p2.uid]), set(p4[CITY.IS_CHILD_OF].keys()))

        # check if items correctly added in second session
        # active relations
        self.assertEqual(set(cw[CITY.HAS_INHABITANT].keys()),
                         set([p1w.uid, p2w.uid]))
        self.assertEqual(set([p2w.uid]),
                         set(p1w[CITY.HAS_CHILD].keys()))
        self.assertEqual(set([p4w.uid]),
                         set(p2w[CITY.HAS_CHILD].keys()))

        # passive relations
        self.assertEqual(set([cw.uid]),
                         set(p1w[CITY.IS_INHABITANT_OF].keys()))
        self.assertEqual(set([p1w.uid]),
                         set(p2w[CITY.IS_CHILD_OF].keys()))
        self.assertEqual(set([cw.uid]),
                         set(p2w[CITY.IS_INHABITANT_OF].keys()))
        self.assertEqual(set([p2w.uid]),
                         set(p4w[CITY.IS_CHILD_OF].keys()))
    ...
    def test_fix_neighbours(self):
        ...
        self.assertEqual(
            set(p1w1[CITY.IS_INHABITANT_OF].keys()),
            set([c1w1.uid, c2w1.uid, c3w1.uid]))
        self.assertEqual(
            set(p2w2[CITY.IS_CHILD_OF].keys()),
            set([p1w2.uid]))
        ...
        self.assertNotIn(CITY.IS_PART_OF, p2w2)

        # check if there are no unexpected other changes
        self.assertEqual(
            set(c1w1[CITY.HAS_INHABITANT].keys()),
            set([p1w1.uid]))
        self.assertEqual(
            set(c2w1[CITY.HAS_INHABITANT].keys()),
            set([p1w1.uid]))
        self.assertEqual(
            set(c3w1[CITY.HAS_INHABITANT].keys()),
            set([p1w1.uid]))
        self.assertNotIn(CITY.HAS_INHABITANT, c4w1)

        # check if the parents in w2 have been updated
        self.assertEqual(
            set(c1w2[CITY.HAS_INHABITANT].keys()),
            set([p1w1.uid]))
        self.assertEqual(
            set(c3w2[CITY.HAS_INHABITANT].keys()),
            set([p1w1.uid]))
        self.assertEqual(
            set(c4w2[CITY.HAS_INHABITANT].keys()),
            set([p1w1.uid]))
    ...
    def test_remove(self):
        ...
        self.assertIn(CITY.HAS_PART, c)
        self.assertIn(CITY.HAS_INHABITANT, c)
        ...
        self.assertNotIn(CITY.HAS_PART, c)
        # inverse
        get_inverse = n.get(rel=CITY.IS_PART_OF)
        self.assertEqual(get_inverse, [])
        ...
        self.assertNotIn(CITY.HAS_INHABITANT, c)
        # inverse
    ...
    #         wrapper = CITY.CITY_WRAPPER(session=session)
    #         cw = wrapper.add(c)
    #         n = CITY.NEIGHBOURHOOD("Zähringen")
    #         nw = cw.add(n)
    ...
    #     c1 = CITY.CITY("Freiburg")
    #     c2 = CITY.CITY("Offenburg")
    #     with CoreSession() as session:
    #         w = CITY.CITY_WRAPPER(session=session)
    #         c1w, c2w = w.add(c1, c2)
