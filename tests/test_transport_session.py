...
CUDS_DICT = [
    {"@id": PRFX + "01", "@type": ["http://www.osp-core.com/city#City"]},
    {"@id": PRFX + "03", "@type": ["http://www.osp-core.com/city#Person"]},
    {"@id": PRFX + "02", "@type": ["http://www.osp-core.com/city#Person"]},
    {
        "@id": PRFX + "7b",
    ...
    "@graph": [
        {
            ...
            "city:name": "Peter",
            "city:age": 23,
            "city:INVERSE_OF_hasInhabitant": {"@id": PRFX + "01"},
            "city:hasChild": [{"@id": PRFX + "02"}, {"@id": PRFX + "03"}],
        },
        {
            "@id": PRFX + "01",
            "@type": "city:City",
            "city:coordinates": {
                "@type": "cuba:_datatypes/VECTOR-INT-2",
                "@value": "[0, 0]",
            },
            "city:name": "Freiburg",
    ...
    [
        {
            ...
            "http://www.osp-core.com/city#coordinates": [
                {
                    "@type": "http://www.osp-core.com/cuba#_datatypes"
                    "/VECTOR-INT-2",
                    "@value": "[0, 0]",
                }
            ],
...
ROOT_DICT = [
    {"@id": PRFX + "01", "@type": ["http://www.osp-core.com/city#City"]},
    {
        ...
        "http://www.osp-core.com/city#hasPart": [{"@id": PRFX + "01"}],
    },
]
...
SERIALIZED_BUFFERS = {
    "added": [
        [
            ...
            },
            {
                ...
                ],  # TODO correct serialization of vector
    ...
    "updated": [
        [
            {
                "@id": PRFX + "7b",
                "http://www.osp-core.com/city#hasPart": [{"@id": PRFX + "02"}],
                "@type": ["http://www.osp-core.com/city#CityWrapper"],
            },
            {
                "@id": PRFX + "02",
...
def jsonLdEqual(a, b):
    ...
    if (
        ...
        return isomorphic(
            json_to_rdf(a, rdflib.Graph()), json_to_rdf(b, rdflib.Graph())
        )
    elif (
        a
...
class TestCommunicationEngineSharedFunctions(unittest.TestCase):
    ...
    def testDeserialize(self):
        ...
        with TestWrapperSession() as session:
            ...
            cuds_objects = deserialize(
                CUDS_LIST_PARTITIONED, session, BufferContext.USER
            )
        ...
        with TestWrapperSession() as session:
            ...
            self.assertEqual(
                set(cuds_object._neighbors.keys()),
                {city.INVERSE_OF_hasInhabitant, city.hasChild},
            )
            self.assertEqual(
            ...
            self.assertEqual(
                cuds_object._neighbors[city.hasChild],
                {
            ...
            self.assertEqual(
                deserialize(None, session, BufferContext.USER), None
            )
            self.assertEqual(
            ...
            self.assertEqual(
                deserialize(
                    {"ENTITY": "city.Citizen"}, session, BufferContext.USER
                ),
                city.Citizen,
            )
            self.assertEqual(
    ...
    def test_serializable(self):
        ...
        assertJsonLdEqual(self, [CUDS_DICT], serializable([p]))
        assertJsonLdEqual(
            self, CUDS_LIST_PARTITIONED, serializable([p, c, c1, c2])
        )
        assertJsonLdEqual(
            self,
            CUDS_LIST_NON_PARTITIONED,
        ...
        assertJsonLdEqual(
            self, CUDS_DICT, serializable([p], partition_cuds=False)
        )
        ...
        assertJsonLdEqual(
            self, {"ENTITY": "city.Citizen"}, serializable(city.Citizen)
        )
        assertJsonLdEqual(
            self,
            [{"ENTITY": "city.Citizen"}, {"ENTITY": "city.City"}],
        ...
        assertJsonLdEqual(
            self, [1, 1.2, "hallo"], serializable([1, 1.2, "hallo"])
        )
    ...
    def test_deserialize_buffers(self):
        ...
        with TestWrapperSession() as s1:
            ...
            self.assertEqual(
                additional, {"args": [42], "kwargs": {"name": "London"}}
            )
            self.assertEqual(
            ...
            self.assertEqual(
                ws1._neighbors[city.hasPart], {cn.uid: [city.City]}
            )
            self.assertEqual(set(ws1._neighbors.keys()), {city.hasPart})
            self.assertEqual(
                cn._neighbors[city.isPartOf], {ws1.uid: [city.CityWrapper]}
            )
    ...
    def test_serialize_buffers(self):
        ...
        with TestWrapperSession() as s1:
            ...
            self.assertEqual(
                s1._buffers[BufferContext.ENGINE], [dict(), dict(), dict()]
            )
            self.maxDiff = None
            result = serialize_buffers(
                s1,
                buffer_context=BufferContext.USER,
                additional_items={"args": [42], "kwargs": {"name": "London"}},
            )
        ...
        with TestWrapperSession() as s1:
            ...
            assertJsonLdEqual(
                self, SERIALIZED_BUFFERS_EXPIRED, json.loads(result[0])
            )
            self.assertEqual([], result[1])
            self.assertEqual(
                s1._buffers,
...
class TestCommunicationEngineClient(unittest.TestCase):
    ...
    def test_store(self):
        ...
        c1 = create_recycle(
            ...
            session=client,
            fix_neighbors=False,
        )  # store will be called here
        self.assertEqual(client._engine._sent_command, INITIALIZE_COMMAND)
        assertJsonLdEqual(
            self, json.loads(client._engine._sent_data), INIT_DATA
        )
    ...
    def test_receive(self):
        ...
        self.assertEqual(
            set(client._registry.keys()), {uuid.UUID(int=42), w.uid}
        )
        self.assertEqual(
            client._buffers[BufferContext.USER], [dict(), dict(), dict()]
        )
        self.assertEqual(
...
class TestCommunicationEngineServer(unittest.TestCase):
    ...
    def test_run_command(self):
        ...
        def command(s, uid, name):
            ...
            correct = (
                set(added.keys()) == {uuid.UUID(int=2)}
                and added[uuid.UUID(int=2)].name == "Paris"
            )
        ...
        with TestWrapperSession() as s1:
            ...
            # test the method
            server.session_objs = {"1": s1, "2": 123}
            result = server._run_command(
                json.dumps(SERIALIZED_BUFFERS), "command", "1"
            )
    ...
    def test_init_session(self):
        ...
        data = json.dumps(
            {"args": ["invalid"], "kwargs": {}, "root": CUDS_DICT}
        )
        self.assertRaises(
            TypeError, server._init_session, data, connection_id="user1"
        )
