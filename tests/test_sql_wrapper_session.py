    ...
    def test_queries_predicate_given(self):
        ...
        self.assertEqual(
            ...
            AndCondition(
                ...
                EqualsCondition(
                    "tp", "name", "activeRelationship", rdflib.XSD.string
                ),
            ),
        )
        self.assertEqual(
        ...
        # type
        r = sorted(
            self.session._queries(pattern=(None, rdflib.RDF.type, None)),
            key=lambda x: x[1],
        )
        self.assertEqual(len(r), 1)
        ...
        # data
        r = sorted(
            self.session._queries(pattern=(None, city.coordinates.iri, None)),
            key=lambda x: x[1],
        )
        self.assertEqual(len(r), 1)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            AndCondition(
                ...
                JoinCondition(
                    data_tbl("VECTOR-INT-2"), "p", "tp", "entity_idx"
                ),
                EqualsCondition("tp", "ns_idx", 2, rdflib.XSD.integer),
                EqualsCondition(
                    "tp", "name", "coordinates", rdflib.XSD.string
                ),
            ),
        )
        self.assertEqual(
...
class MockSqlWrapperSession(SqlWrapperSession):
    ...
    def _db_insert(self, table_name, columns, values, datatypes):
        """Do nothing."""
        if (
            table_name == NAMESPACES_TABLE
            and columns == ["namespace"]
    ...
    def _db_select(self, query):
        """Do nothing."""
        if list(query.tables.values()) == [CUDS_TABLE] and list(
            query._columns.values()
        ) == [["cuds_idx", "uid"]]:
            yield int(query.condition.value[-1]), query.condition.value
        elif list(query.tables.values()) == [ENTITIES_TABLE] and list(
            query._columns.values()
        ) == [["entity_idx", "ns_idx", "name"]]:
            yield 42, 2, "hasInhabitant"
        elif list(query.tables.values()) == [NAMESPACES_TABLE] and list(
            query._columns.values()
        ) == [["ns_idx", "namespace"]]:
