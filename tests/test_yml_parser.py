...
class TestYMLParser(unittest.TestCase):
    ...
    def test_set_datatype(self):
        ...
        self.parser._set_datatype(x)
        self.assertEqual(
            set(self.parser._graph),
            {
                (
                    self.parser._get_iri(x),
                    rdflib.RDFS.range,
    ...
    def test_check_default_rel_flag_on_entity(self):
        ...
        self.parser._check_default_rel_flag_on_entity(
            "relationshipB", self.ontology_doc["relationshipB"]
        )
        self.assertEqual(list(self.parser._graph), [])
        self.parser._check_default_rel_flag_on_entity(
            "relationshipA", self.ontology_doc["relationshipA"]
        )
    ...
    def test_set_inverse(self):
        ...
        self.assertEqual(list(self.parser._graph), [])
        self.parser._set_inverse(
            "relationshipC", self.ontology_doc["relationshipC"]
        )
        self.assertEqual(
            list(self.parser._graph),
            [
                (
                    self.parser._get_iri("relationshipC"),
    ...
    def test_add_attributes(self):
        ...
        self.assertRaises(
            ValueError,
            self.parser._add_attributes,
            "ClassA",
            self.ontology_doc["ClassA"],
        )
        self.parser._parse_entity(
            "attributeA", self.ontology_doc["attributeA"]
        )

        # with default
        pre = set(self.parser._graph)
        self.parser._add_attributes("ClassA", self.ontology_doc["ClassA"])
        bnode1 = self.parser._graph.value(
            self.parser._get_iri("ClassA"), rdflib_cuba._default
        )
        bnode2 = self.parser._graph.value(
            None, rdflib.RDF.type, rdflib.OWL.Restriction
        )
        self.assertEqual(
            set(self.parser._graph) - pre,
            {
                (self.parser._get_iri("ClassA"), rdflib_cuba._default, bnode1),
                (
                    bnode1,
                    rdflib_cuba._default_value,
                    rdflib.Literal("DEFAULT_A"),
                ),
                (
                    bnode1,
                    rdflib_cuba._default_attribute,
        ...
        for s, _, _ in self.parser._graph.triples(
            (None, rdflib.RDF.type, rdflib.OWL.Restriction)
        ):
            if s != bnode2:
                bnode3 = s
    ...
    def test_add_superclass(self):
        ...
        self.parser._add_superclass(
            "relationshipB", iri, "parser_test.relationshipA"
        )
        iri = self.parser._get_iri("attributeB")
        self.parser._add_superclass(
            "attributeB", iri, "parser_test.attributeA"
        )
