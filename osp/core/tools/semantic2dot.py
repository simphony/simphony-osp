...
from osp.core.ontology import OntologyEntity,\
    OntologyAttribute, \
    OntologyClass, \
    OntologyIndividual, OntologyNamespace, OntologyRelationship
...
class Semantic2Dot:
    ...
    _label = ("<<TABLE BORDER='0' CELLBORDER='0'>"
              "<TR><TD>{}</TD></TR>"
              "{}"
              "</TABLE>>")
    ...
    def __init__(self,
                 *elements: Union[OntologyIndividual,
                                  OntologyNamespace,
                                  Session,
                                  # Wrapper, # TODO,
                                  ],
    ...
    @staticmethod
    def _element_label(element: Union[OntologyEntity,
                                      Session,
                                      # Wrapper, # TODO
                                      OntologyNamespace]):
    ...
    def _add_ontology_entity(self,
                             entity: Union[OntologyClass,
                                           OntologyIndividual,
        ...
        entity_namespace = next((ns for ns in entity.session.namespaces
                                 if entity in ns), None)
    ...
    def _add_node_individual(self, individual: OntologyIndividual):
        ...
        if individual in self._elements:
            ...
            label = self._label.format(self._element_label(individual),
                                       attributes)
            self._graph.node(str(individual.identifier)
                             .replace(':', '_').replace('/', '_'),
                             label=label,
                             color="lightblue",
                             style="filled")
    ...
    def _add_edge_individual_relationship(self, start, end, relationship):
        ...
        """
        self._graph.edge(start.identifier
                         .replace(':', '_').replace('/', '_'),
                         end.identifier
                         .replace(':', '_').replace('/', '_'),
    ...
    def _add_edge(self, start: OntologyEntity, end: OntologyEntity, **kwargs):
        ...
        """
        self._graph.edge(str(start.identifier)
                         .replace(':', '_').replace('/', '_'),
                         str(end.identifier)
                         .replace(':', '_').replace('/', '_'),
                         **kwargs)

    def _add_node_oclass(self,
    ...
    def _add_node_attribute(self,
        ...
        graph.node(str(attribute.identifier)
                   .replace(':', '_').replace('/', '_'),
                   label=label,
                   color="#7EB874",
                   style="filled")
