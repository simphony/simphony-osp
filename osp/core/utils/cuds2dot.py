...
class Cuds2dot():
    ...
    def __init__(self, root):
        ...
        self._root = root
        self._visited = set()
        self._root_uid = self.shorten_uid(root.identifier)
        self._graph = self._initialize_graph()

    def _initialize_graph(self):
        """Initialize a directed graph with some default settings."""
        graph = graphviz.Digraph(format='png', name=str(self._root.identifier))
    ...
    def render(self, filename=None, **kwargs):
        ...
        self._visited.add(self._root.identifier)
    ...
    def _add_directly_related(self, current):
        ...
        current_uid = self.shorten_uid(current.identifier)
        for cuds_object in current.iter(rel=cuba.relationship):
            cuds_object_uid = self.shorten_uid(cuds_object.identifier)
            ...
            # Add the node if new
            if cuds_object.identifier not in self._visited:
                self._add_node(cuds_object, cuds_object_uid)
                self._visited.add(cuds_object.identifier)
