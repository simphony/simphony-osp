...
emmo_town.add(city.Citizen(name='Emanuele Ghedini', age=15),
              rel=city.hasInhabitant)
emmo_town.add(city.Citizen(name='Adham Hashibon', age=15),
              rel=city.hasInhabitant)
emmo_town.add(city.Citizen(name='Jesper Friis', age=15),
              city.Citizen(name='Gerhard Goldbeck', age=15),
              city.Citizen(name='Georg Schmitz', age=15),
              city.Citizen(name='Anne de Baas', age=15),
              rel=city.hasInhabitant)

emmo_town.add(city.Neighborhood(name="Ontology", coordinates=[1, 2]))
emmo_town.add(city.Neighborhood(name="User cases", coordinates=[3, 4]))

ontology_uid = None
for neighborhood in emmo_town.get(oclass=city.Neighborhood):
    if neighborhood.name == "Ontology":
        ontology_uid = neighborhood.uid
        neighborhood.add(city.Street(name="Relationships",
                                     coordinates=[5, 6]), rel=city.hasPart)
        neighborhood.add(city.Street(name="Entities",
                                     coordinates=[7, 8]), rel=city.hasPart)
